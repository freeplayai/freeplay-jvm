...
public class AnthropicWithMediaTest {
    String anthropicApiKey = requireEnv("ANTHROPIC_API_KEY");
    ...
    public void testWithImage() throws IOException, URISyntaxException, ExecutionException, InterruptedException {
        ...
        byte[] base64bytes = Base64.getEncoder().encode(Files.readAllBytes(imageFile.toPath()));
        ...
        var response = fetchCompletionResponse(formattedPrompt);
        assertEquals(200, response.statusCode());

        var bodyNode = objectMapper.readTree(response.body());
        String responseContent = bodyNode.get("content").get(0).get("text").asText();
    ...
    public void testWithDocument() throws IOException, URISyntaxException, ExecutionException, InterruptedException {
        ...
        MediaInputCollection media = new MediaInputCollection();
        File imageFile = new File(this.getClass().getClassLoader().getResource("media/portugal.pdf").toURI());
        byte[] base64bytes = Base64.getEncoder().encode(Files.readAllBytes(imageFile.toPath()));
    ...
    private HttpResponse<String> fetchCompletionResponse(FormattedPrompt<List<ChatMessage>> formattedPrompt) throws URISyntaxException, IOException, InterruptedException {
        var requestBuilder = HttpRequest
                .newBuilder(new URI("https://api.anthropic.com/v1/messages"))
                .header("Content-Type", "application/json")
                .header("anthropic-version", "2023-06-01")
                .header("x-api-key", anthropicApiKey)
                .POST(ofString(objectMapper.writeValueAsString(
                        Map.of(
                                "system", formattedPrompt.getSystemContent().get(),
